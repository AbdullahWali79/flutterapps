...
class dice_6 extends StatefulWidget {
  const dice_6({Key? key}) : super(key: key);

  @override
  _dice_6State createState() => _dice_6State();
}

class _dice_6State extends State<dice_6> with SingleTickerProviderStateMixin {
  int a=1;
  int b=1;
  int c=1;
  int d=1;
  ...
  animate(){
    ...
    animation.addStatusListener((status) {if(status==AnimationStatus.completed){
      setState(() {
        ...
        f=Random().nextInt(6)+1;
        tvalue=tvalue+a+b+c+d+e+f;
      });
      print("complete");
      _controller.reverse();
    }
